  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input type="radio" class="btn-check" name="dispo_pourcent" id="dispo_0_{$date.id}_{$membre_id}" autocomplete="off" value="0"{if $dispo_pourcent == 0} checked{/if}>
        <label class="btn btn-danger" for="dispo_0_{$date.id}_{$membre_id}"><i class="fa fa-circle-xmark me-3"></i>Non</label>

        <input type="radio" class="btn-check" name="dispo_pourcent" id="dispo_50_{$date.id}_{$membre_id}" autocomplete="off" value="50" {if $dispo_pourcent == 50} checked{/if}>
        <label class="btn btn-light" for="dispo_50_{$date.id}_{$membre_id}"><i class="fa fa-circle-question me-3"></i>Peut-être</label>

        <input type="radio" class="btn-check" name="dispo_pourcent" id="dispo_100_{$date.id}_{$membre_id}" autocomplete="off" value="100" {if $dispo_pourcent == 100} checked{/if}>
        <label class="btn btn-success" for="dispo_100_{$date.id}_{$membre_id}"><i class="fa fa-circle-check me-3"></i>Oui</label>

        <textarea name="dispo_commentaire" class="form-control my-4" placeholder="Une envie ou une excuse bidon ?">{$dispo_commentaire}</textarea>
