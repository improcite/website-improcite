...
function updateEvenement($mysqli, $table, $data) {
    $replace = "REPLACE INTO $table (id, categorie, date, commentaire, lieu, tarif, places, joueurs, coach, mc, arbitre, regisseur, caisse, animateurs, lien_facebook, lien_mobilizon) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $query = $mysqli->execute_query($replace, array($data["id"], $data["categorie"], $data["date"], $data["commentaire"], $data["lieu"], $data["tarif"], $data["places"], $data["joueurs"], $data["coach"], $data["mc"], $data["arbitre"], $data["regisseur"], $data["caisse"], $data["animateurs"], $data["lien_facebook"], $data["lien_mobilizon"]));
    if (!$query && $debug) {
        die($mysqli->sqlstate);
    }
    return $query;
...
function createEvenement($mysqli, $table, $data) {
    $insert = "INSERT INTO $table (categorie, date, commentaire, lieu, tarif, places, joueurs, coach, mc, arbitre, regisseur, caisse, animateurs, lien_facebook, lien_mobilizon) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $query = $mysqli->execute_query($insert, array($data["categorie"], $data["date"], $data["commentaire"], $data["lieu"], $data["tarif"], $data["places"], $data["joueurs"], $data["coach"], $data["mc"], $data["arbitre"], $data["regisseur"], $data["caisse"], $data["animateurs"], $data["lien_facebook"], $data["lien_mobilizon"]));
    if (!$query && $debug) {
        die($mysqli->sqlstate);
    }
    return $query;
